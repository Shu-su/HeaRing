...
지도는 **네이버 지도 API** 를, 알림 서비스는 **Firebase Cloud Messaging(FCM)** 을 사용합니다. <br>
따라서 지도와 알림 서비스를 사용하기 위해 별도로 API 키 발급과 FCM 토큰 생성 작업이 필요로 합니다. <br>
...
- `web_socket_channel`: 실시간 통신을 위한 웹소켓 라이브러리 (실시간 위치를 확인하기 위해 사용)
