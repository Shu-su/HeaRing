...
## 목차
1. [프로젝트 개요](#1-개요)
2. [개발 전 원격제어 환경 구성](#2-개발-전-원격제어-환경-구성-방법 )
3. [사용한 센서 및 부가적인 부품과 적용 방법 ](#3-사용한-센서-및-부가적인-부품과-적용방법)
4. [하드웨어 로직 & 코드 구성 및 이슈해결 방법 ](#4 )
5. [결론](#5)
...
- 추가적인 소형화 개발을 성공적으로 진행 하기 위한 Raspberry Pi Zero 2W 
  - 제로 2W 부품은, 소형화 개발의 목표에 매우 적합한 장비였지만 성능이 매우 떨어져 우리 팀이 구현해야하는 기능들이 원활하게 개발되어 작동될 것 같지 않다고 판단하였다
...
## 병렬 구조 설계의 주요 고민
  ...
  -  전체적인 구조:<br>
    ...
    동시에, 실시간 위치 데이터는 AWS Gateway API와 웹소켓(WebSocket)을 활용하여 양방향 통신으로 처리되야함. 백그라운드에서는 MQTT와 웹소켓이 활성화되어 서버로부터 실시간 위치 데이터를 요청하는 신호를 대기하고, 요청이 감지되면 해당 데이터를 전송하거나 중지하는 작업이 원활하게 이루어져야한다  <br>
...
<br> 
초기 단계에서 구현이 어려울 것이라고 예상했던 기능들을 완성하면서 많은 도전과 학습의 기회가 되었고, 이를 통해 실질적인 기술적 성장과 자신감을 얻게 되었음
