...
## 병렬 구조 설계의 주요 고민
  ...
  -  전체적인 구조:<br>
    ...
    동시에, 실시간 위치 데이터는 AWS Gateway API와 웹소켓(WebSocket)을 활용하여 양방향 통신으로 처리되야함. 백그라운드에서는 MQTT와 웹소켓이 활성화되어 서버로부터 실시간 위치 데이터를 요청하는 신호를 대기하고, 요청이 감지되면 해당 데이터를 전송하거나 중지하는 작업이 원활하게 이루어져야함 <br>
...
## 5. 결론 
이 프로젝트를 통해 라즈베리파이 기반의 실시간 데이터 처리 시스템을 성공적으로 개발했습니다. 특히, 음성 녹음과 위치 데이터를 병렬적으로 처리하며, AWS S3와 EC2 서버를 활용한 데이터 업로드와 실시간 통신이 원활히 구현되었다. 추가적으로 전혀 구현 하지 못했을 것이라고 생각했던 부분이 구현 되었 던 것이 개발  한 후 가장 큰 성과라고 생각한다 

프로젝트의 하드웨어 부분의 핵심 성과 정리:
- Safe 존 기반 외출 여부 판단 및 녹음 데이터 처리.
- AWS 서비스를 활용한 효율적인 데이터 관리와 확장성 있는 구조 설계.
- 실시간 위치 데이터를 안정적으로 처리하는 IoT 시스템 구현.

<br><br>
앞으로의 추가적인 확장성과 한계점보완 방법 계획 <br> 
시스템의 안정성을 더욱 강화하고, 추가적인 센서들을 도입하여, 단말기가 몸에서 떨어졌을 때 위험 상황의 자동 감지와 같은 기능을 추가할 계획이다 
소형화 계획을 실패 하였으니 추후엔 소형화 계획을 성공 시킬 수 있도록 진행 할 예정이다 
